...
class AI:
	...
	def __init__(self, inputShape=(13,8,8)):
	...
	def getGame(self, file, returnBoards=False):
		pgn = open(cwd + '/dataset/' + file + '.pgn') # 125567 games
		# pgn = open(cwd + '/checkmateonly2.pgn') # x games
		while True:
			boardSet = []
			winners = []
			policyFromMoves = []
			...
			for i in range(30):
				...
				for moveNum, move in enumerate(moves):
					...
					policyFromMove = np.zeros((64,))
					policyToMove = np.zeros((64,))
					if (moveNum + 1 < len(moves)):
						policyFromMove[moves[moveNum+1].from_square] = 1
						policyToMove[moves[moveNum+1].to_square] = 1

					boardSet.append(buildInput(board))
					winners.append(result if board.turn else result * -1)
					policyFromMoves.append(policyFromMove)
					policyToMoves.append(policyToMove)
					# print(chosenMove)
			...
			if not returnBoards:
				yield np.array(boardSet), [np.array(winners), np.array(policyFromMoves), np.array(policyToMoves)]
			else:
				yield (np.array(boardSet), [np.array(winners), np.array(policyFromMoves), np.array(policyToMoves)], boards)
	...
	def newModelConnect4(self):
		...
		x = Conv2D(64, (4,4), kernel_regularizer=l2(kernel_reg), kernel_initializer=kernel_init, padding='same', data_format='channels_first', use_bias=False)(x)
		...
		def residualLayers(inputLayering, count):
			...
			x = Conv2D(64, (3,3), kernel_regularizer=l2(kernel_reg), kernel_initializer=kernel_init, padding='same', data_format='channels_first', use_bias=False)(inputLayering)
		...
		resid = residualLayers(x, 5)


		x = Conv2D(32, (6,7), kernel_regularizer=l2(kernel_reg), kernel_initializer=kernel_init, padding='same', data_format='channels_first', use_bias=False)(resid)
		...
		def policy(inputLayering, name):
			x = Conv2D(32, (6,7), kernel_regularizer=l2(kernel_reg), kernel_initializer=kernel_init, padding='same', data_format='channels_first', use_bias=False)(inputLayering)
			...
			x = Dense(7, use_bias=False, kernel_regularizer=l2(kernel_reg), kernel_initializer=kernel_init_dense)(x)
	...
	def trainHuman(self, file):
		...
		self.model.fit_generator(self.getGame(file), steps_per_epoch=250, epochs=16, callbacks=[MyCustomCallback()])
